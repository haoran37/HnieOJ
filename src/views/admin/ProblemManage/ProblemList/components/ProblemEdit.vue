      ...
      <n-form ref="formRef" :model="problem" :rules="rules" label-placement="left" label-width="120px">
        ...
        <n-grid :cols="2" :x-gap="24">
          <n-form-item-gi label="时间限制 (ms)">
            <n-input-number v-model:value="problem.problem.timeLimit" :min="0" placeholder="1000" />
          </n-form-item-gi>
          <n-form-item-gi label="空间限制 (MB)">
            <n-input-number v-model:value="problem.problem.memoryLimit" :min="0" placeholder="256" />
          </n-form-item-gi>
        </n-grid>
        ...
        <n-form-item label="OI总分" v-if="problem.problem.type === 1">
          <n-input-number v-model:value="problem.problem.ioScore" :min="0" placeholder="100" />
        </n-form-item>
...
onMounted(() => {
  ...
  if (id) {
    // 模拟获取题目详情
    console.log('Fetch problem detail for id:', id);
    // 这里应该调用API获取题目详情并填充到 problem 对象中
    problem.problem.title = '模拟编辑题目';
    problem.problem.problemId = String(id);
  }
});
</script>
