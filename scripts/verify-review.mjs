...
// 运行：node scripts/verify-review.mjs（仓库根目录，Node 20.19 / 22.12 / 24 直接执行，无需 flags）
//
// Node 20.19/22.12 既没有原生 TS 类型剥离也没有 registerHooks：统一用已有 devDependency jiti
// 转译被测 TS 模块；SFC 仍由 vue/compiler-sfc 真实编译，再用 jiti 求值。
// UI/网络桩写入 os 临时目录，脚本结束（含失败）时在 finally 精确清理本任务创建的临时目录。
...
// 先加载真实 vue（CJS 入口，jiti 转译模块 require('vue') 时命中同一 Node 缓存），
// 保证脚本创建的 reactive/watch 与被测代码共享同一套响应式系统。
const vue = require('vue');
const { createRenderer, defineComponent, reactive } = vue;

// ---------------- UI / 网络桩（os 临时目录，结束时清理） ----------------
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'verify-review-'));
...
const naiveUiStub = writeStub('naive-ui-stub.mjs', [
...
const jiti = createJiti(import.meta.url, {
  ...
  alias: {
    '@': srcRoot,
    '@/utils/api': apiStub,
    'naive-ui': naiveUiStub,
    'vue-router': vueRouterStub,
    '@vicons/ionicons5': viconsStub,
    './components/DiscussAnswerItem.vue': answerItemStub,
  },
});

const importTs = (relativePath) => jiti.import(path.join(srcRoot, relativePath));

// 真实编译 SFC <script setup>，再用 jiti 求值（模块内 import 走同一 alias/同一 vue 实例）
async function compileSfcModule(relativePath, id) {
  ...
  return jiti.evalModule(code, { filename: file, async: false });
}
